...
def mean_pooling(model_output, attention_mask):
    ...
    input_mask_expanded = np.broadcast_to(
        np.expand_dims(attention_mask.cpu(), -1), token_embeddings.shape
    )
...
def create_document_record(
    file_name: str, file_size: int, file_type: str, user_id: int = 1
) -> int:
    """
...
def save_chunks_to_db(chunks: List[str], document_id: int):
    ...
    try:
        ...
        logger.info(
            f"Начинаем сохранение {len(chunks)} чанков для документа {document_id}"
        )
...
def add_text_to_database(
    text: str, file_name: str, file_size: int, file_type: str, user_id: int = 1
):
    """
    ...
    try:
        ...
        document_id = create_document_record(
            file_name, file_size, file_type, user_id=user_id
        )
        ...
        logger.info(
            f"=== УСПЕШНО ЗАВЕРШЕНО: добавлено {len(chunks)} чанков для документа {document_id} ==="
        )
        return len(chunks)
